...
#$ImgSelection | Out-GridView -PassThru -Title "Select the desired Windows Image from the list"    
...
$var_WinImage_cmb.Add_SelectionChanged({
        ...
        $global:DeployProgram = $ImgSelection.DeployProgram	#Deployment Program to be used
        $global:ImageType = $ImgSelection.ImageType # Type of Image Selected

		#region Creating $ImgSelection.XMLFilePath and Assinging it to $global:ImageXMLPath        
		Copy-Item -Force -Path $ImgSelection.XMLFilePath -Destination $(Join-Path $env:TEMP -ChildPath $(Split-Path -Path $ImgSelection.XMLFilePath -Leaf -Resolve))        
...
$var_Install_btn.Add_Click({
		try
		{
			...
			else
			{
                ...
                switch($global:ImageType)
                {
                    'GHO' {
                    $global:SelectedDisk += 1
                    $WinPartition = Get-Partition -DriveLetter W | Select-Object -ExpandProperty PartitionNumber 
                    Write-Host "Windows DISK Number: $global:SelectedDisk"
                    Write-Host "Windows Parition Number: $WinPartition"
                    Write-Host "Windows Destination: $($global:SelectedDisk):$WinPartition"
                    Write-Host "Windows ImageFile Path: $($global:ImageFilePath)"
                    Write-Host "Unattneded XML File Path: $($global:ImageXMLPath)"
                    $global:ImageXMLPath                    
                    $DeployArgumentList = "-clone,MODE=prestore,SRC=$($global:ImageFilePath):1,DST=$($global:SelectedDisk):$WinPartition -sure"                                            
                    Write-Host "Starting Windows Installation NOW..." -ForegroundColor Red
                    
                    Write-Host "1. Deploying Ghost Image" -ForegroundColor Cyan
                    $RunSetup = Start-Process -Wait $($global:DeployProgram) -ArgumentList $DeployArgumentList -ErrorAction Stop                    

                    Write-Host "2. Replacing Unattend XML File with $($global:ImageXMLPath)" -ForegroundColor Cyan
                    Copy-Item -Path $global:ImageXMLPath -Destination 'W:\Windows\Panther\unattend.xml' -Force -ErrorAction SilentlyContinue

                    Write-Host "3. Adding Drivers $($global:DriverPath) to Windows (W:\)" -ForegroundColor Cyan 
                    Add-WindowsDriver -Recurse -ForceUnsigned -Path "W:\" -Driver $global:DriverPath

                    Write-Host "4. Writing Boot Record for $($global:SelectedPartScheme) Scheme" -ForegroundColor Cyan
                    Start-Process -Wait -FilePath 'BCDBOOT' -ArgumentList "W:\Windows /S S: /F $global:SelectedPartScheme"
                               

                    Write-Host "Installation DONE!" -ForegroundColor Red

                    break
                    }

                    'wim' {
                    $DeployArgumentList = "NT6 /source:$global:ImageFilePath /sysPart:S: /tempDrive:W: /wimIndex:1 /silent /setup "
				    if ($global:ImageXMLPath) { $DeployArgumentList += "/unattend:$global:ImageXMLPath " }
				    if ($global:DriverPath) { $DeployArgumentList += "/drivers:$global:DriverPath " }                
                    Write-Debug "XML FILE: $global:ImageXMLPath"
				    Write-Debug "XML FILE: $global:DriverPath"
				    Write-Debug "Running $($global:DeployProgram) $DeployArgumentList"
				...
				$global:AllInfo | Export-Clixml -Path $(Join-Path 'W:\SWSetup' -ChildPath "$(Get-WmiObject Win32_Bios | Select-Object -ExpandProperty SerialNumber).xml")
				...
				Write-Host "Updating for 254.14 Server"
				Get-ChildItem -Path W:\SWSetup -Filter *.ps1 -ErrorAction SilentlyContinue | `
				ForEach-Object {
					Write-Host Write-Host $_.FullName
					(Get-Content -Path $_.FullName -ErrorAction SilentlyContinue) -replace '155.180.254.14','155.180.254.13'|Set-Content -Path $_.FullName -Force -ErrorAction SilentlyContinue
					}
}
				...
				if (Test-Path "$global:DriverPath\BIOS")
				{
                 ...
                 #Write-Host "5. Updating for 254.14 Server"
                #('W:\SWSetup\Continue-Install.ps1','W:\SWSetup\LF-Installation.ps1')|ForEach-Object {((Get-Content -Path $_ -ErrorAction SilentlyContinue ) -replace '155.180.254.14','155.180.254.13')|Set-Content -Path $_ -Force -ErrorAction SilentlyContinue}
				Write-Host "Installation Completed. Restart Computer Now. Please Remove USB Storage Drive"
				Start-Sleep -Seconds 90 -ErrorAction SilentlyContinue
				Restart-Computer -Delay 60 -Force -Verbose
